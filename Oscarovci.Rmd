...
Cilj ove analize je ispitati postoji li povezanost između filmskih žanrova i vjerojatnosti osvajanja Oscara. Analizu provodimo na dva načina:
...
## 1. Single-label analiza (Main Genre)
...
## 2. Multi-label analiza (Movie Genre)
...
Prvo ćemo analizirati prvi pristup: **1. Single-label analiza**
...
```{r}
fisher.test(kontigencijska_single, simulate.p.value = TRUE, B = 1e6)
```  

Zatim pristupamo s **2. Multi-label analizom**: 
